...
def __choose_knowledge_base(subject: Subject, region: Region):
    ...
    if subject_path is None or region_path is None:
        subject_path = 'modern_romance'
        region_path = 'western_realistic'
    return os.path.join(FEMALE_ORIENTED_KB_ROOT, subject_path, region_path)
...
def simple_cover_gen(req: CoverGenReq) -> dict:
    def from_b64_image_to_cover(b64_image: str) -> str:
        ...
        return SimpleTitleGeneration(
            ...
            title_color=tuple(req.title_color) if isinstance(req.title_color, list) else req.title_color,
            title_font=__choose_title_font(req.title, req.novel),
            title_height_correction=req.title_height_correction
        ).generate()

    novel_content = f'《{req.title}》\n{req.novel}'
    kb_path = __choose_knowledge_base(*NovelThemeRecognition(novel=novel_content).recognize())
    log.debug(f'knowledge base: {kb_path}')
...
def diffuser_titled_cover_gen(req: TitledCoverGenReq):
    def from_b64_image_to_titled_cover(b64_image: str) -> str:
        nonlocal req
        return DiffuserTitledCoverGeneration(title=req.title, base64_image=b64_image, image_format='png').generate()
